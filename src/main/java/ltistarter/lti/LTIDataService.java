...
public class LTIDataService {
    ...
    public boolean loadLTIDataFromDB(LTI3Request lti) {
        ...
        sb.append(" FROM LtiKeyEntity k " +
                ...
                "LEFT JOIN c.memberships m " + // LtiMembershipEntity
                "LEFT JOIN m.user u ON u.userSha256 = :user " +// LtiUserEntity
                "LEFT JOIN k.lti3KeyEntity k3 ON k3.id = k.id " // Key3
        );
        ...
        sb.append(" WHERE k3.clientId = :clientId AND k3.deploymentId = :deploymentId AND (m IS NULL OR (m.context = c AND m.user = u))");
        ...
        Query q = repos.entityManager.createQuery(sql);
        q.setMaxResults(1);
        q.setParameter("clientId", lti.aud);
        q.setParameter("deploymentId", lti.ltiDeploymentId);
        q.setParameter("context", BaseEntity.makeSHA256(lti.ltiContextId));
        q.setParameter("link", BaseEntity.makeSHA256(lti.ltiLinkId));
        q.setParameter("user", BaseEntity.makeSHA256(lti.lti11LegacyUserId));
        //TODO
        //if (includesService) {
        //    q.setParameter("service", BaseEntity.makeSHA256(lti.ltiServiceId));
        //}
        ...
        List<Object[]> rows = q.getResultList();
        if (rows == null || rows.isEmpty()) {
            log.info("LTIload: No lti.results found for client_id: " + lti.aud + " and  deployment_id:" + lti.ltiDeploymentId);
        } else {
            // k, c, l, m, u, s, r
            Object[] row = rows.get(0);
            if (row.length > 0) lti.key = (LtiKeyEntity) row[0];
            ...
            lti.loaded = true;
            log.info("LTIload: loaded data for clientid= " + lti.aud + " deploymentid=" + lti.ltiDeploymentId + " and context=" + lti.ltiContextId + ", complete=" + lti.complete);
        }
        return lti.loaded;
    }
    ...
    public int updateLTIDataInDB(LTI3Request lti) {
        ...
        if (lti.user == null && lti.sub != null) {
            LtiUserEntity newUser = new LtiUserEntity(lti.sub, null);
            newUser.setDisplayName(lti.ltiName);
            newUser.setEmail(lti.ltiEmail);
            lti.user = repos.users.save(newUser);
            inserts++;
            log.info("LTIupdate: Inserted user id=" + lti.sub);
        } else if (lti.user != null) {
            repos.entityManager.merge(lti.user); // reconnect object for this transaction
            lti.sub = lti.user.getUserKey();
            lti.ltiName = lti.user.getDisplayName();
            lti.ltiEmail = lti.user.getEmail();
            log.info("LTIupdate: Reconnected existing user id=" + lti.sub);
        }

        if (lti.membership == null && lti.context != null && lti.user != null) {
            int roleNum = LTI3Request.makeUserRoleNum(lti.ltiRoles); // NOTE: do not use userRoleNumber here, it may have been overridden
            ...
            inserts++;
            log.info("LTIupdate: Inserted membership id=" + newMember.getMembershipId() + ", role=" + newMember.getRole() + ", user=" + lti.sub + ", context=" + lti.ltiContextId);
        ...
        */
        ...
        if (lti.ltiName != null && lti.user != null && !lti.ltiName.equals(lti.user.getDisplayName())) {
            lti.user.setDisplayName(lti.ltiName);
        }
        if (lti.ltiEmail != null && lti.user != null && !lti.ltiEmail.equals(lti.user.getEmail())) {
            lti.user.setEmail(lti.ltiEmail);
        }
        if (userChanged) {
            lti.user = repos.users.save(lti.user);
            updates++;
            log.info("LTIupdate: Updated lti.user (id=" + lti.user.getUserKey() + ") name=" + lti.ltiName + ", email=" + lti.ltiEmail);
        }

        if (lti.ltiRoles != null && lti.userRoleNumber != lti.membership.getRole()) {
            lti.membership.setRole(lti.userRoleNumber);
            lti.membership = repos.members.save(lti.membership);
            updates++;
            log.info("LTIupdate: Updated membership (id=" + lti.membership.getMembershipId() + ", user=" + lti.sub + ", context=" + lti.ltiContextId + ") roles=" + lti.ltiRoles + ", role=" + lti.userRoleNumber);
        }
